...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    The `geom_bar()` function in ggplot2 creates a bar chart where the height of the bar is proportional to the number of cases in each group. On the other hand, `geom_col()` creates a bar chart where the heights of the bars represent values in the data. If the plot created with `geom_bar()` is identical to the plot created with `geom_col()`, it means that the heights of the bars in the `geom_bar()` chart are already proportional to the values in the data, and therefore, using `geom_col()` would not change the plot.
